...
public class ExcelUtils {
     ...
     * 每一个sheet的最大可识别行数
     **/
//    private static final int DEFAULT_EXCEL_ROW_NUM = 100;
     ...
     * @param in
     * @param clazz
     * @param startRow 标题所在行 例如：标题在第二行 设置index=2；
     * @throws Exception
     */
    ...
    public static <T> boolean writeToExcel(OutputStream out, Class<T> clazz, List<T> list) {
        return writeToExcel(out, clazz, list, null);
    }
     ...
     * @param response
     * @param clazz
     * @param fileNamePrefix  文件名称
     * @param specialTitleContent 特殊表头
     * @param columnNoList 需要指定为文本格式的列号列表 从0开始
     * @param <T>
     */
    public static <T> void downloadExcelForTemplate(HttpServletResponse response, Class<T> clazz, String fileNamePrefix, String specialTitleContent, List<Integer> columnNoList) {
        try {
            response.setContentType("application/request-data");
            String fileName = new String(fileNamePrefix.getBytes("utf-8"), "ISO8859-1");
            response.setHeader("Content-Disposition", "attachment;filename=" + fileName + JodaTimeUtils.dateToStr(new Date(), JodaTimeUtils.DateFormat.DATE_FORMAT) + ".xlsx");

            Workbook workbook = writeToWorkbookForTemplate(clazz,specialTitleContent, columnNoList);
     ...
     * @param <T>                 数据列中-对应的数据对象
     * @param specialTitleContent 特殊表头
     * @param columnNoList 指定某列为 文本格式  0为第一列
     * @return 返回成功或失败
     */
    ...
    public static <T> Workbook writeToWorkbookForTemplate(Class<T> clazz, String specialTitleContent,List<Integer> columnNoList) {
        ...
        if (rowNumber == 0 || rowNumber % DEFAULT_EXCEL_ROW_NUM == 0) {
            ...
            //设置指定列为文本格式
            if (CollectionUtils.isNotEmpty(columnNoList)) {
                for (Integer columnNo : columnNoList) {
                    sheet.setDefaultColumnStyle(columnNo, defaultCellStyle);
                }
            }
    ...
    public static <T> boolean writeToExcel(OutputStream out, Class<T> clazz, List<T> list, String specialTitleContent) {
        ...
        Workbook workbook = writeToWorkbook(clazz, list, specialTitleContent);
        if (null == workbook) {
            return false;
        }
        boolean returnVal = false;
